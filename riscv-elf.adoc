...
This relocation is needed because code relaxation can shrink preceding code during the linking process, which may
cause an aligned location to become mis-aligned.

To ensure the linker can always satisfy the required alignment solely by deleting bytes, the assembler must emit a
`R_RISCV_ALIGN` relocation and then insert `N - IALIGN` padding bytes before
