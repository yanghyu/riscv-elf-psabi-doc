...
`nop` or `c.nop` if possible, assembler will insert padding bytes which might
be invalid instruction encoding when align a location to 4-bytes or 2-byte if
the C extension is enabled, those padding byte might result illegal instruction
exceptions at run-time if program execute to tha location, but that is not
expected to happen since RISC-V require instruction align to 4-byte or 2-byte
if the C extension is enabled.
