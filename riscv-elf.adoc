...
bytes; this relocation is only required if the containing section
has any `R_RISCV_RELAX` relocations.

The reason why we need a special relocation for handing alignment is because
code relaxation might shrink the code during the linking process, that may
cause an aligned point become mis-aligned after code relaxation.

In order to let the linker have enough space to make sure the alignment
requirement can be met after code relaxation, the assembler must insert a
`R_RISCV_ALIGN` relocation and then insert `N` - 4 padding bytes or `N` - 2
padding bytes if the C extension is enabled, the padding bytes insert before
the location of the alignment diretive like `.align`, `.p2align` or `.balign`,
the addend value of that relocation is the number of padding bytes;
linker may remove arbitrary numbers of the padding bytes at the linking process
to meet the alignment requirement.

The assembler and linker must ensure padding bytes are valid instructions like
`nop` or `c.nop`, assembler should insert null byte if the location isn't
aligned to at least 2-bytes align, the only exception is when the location want
to align is only aligned to 1 byte, assembler may padding null byte, however
null padding byte might result illegal instruction exceptions at run-time if
program execute to tha location, but that is not expected to happen since RISC-V
require instruction align to 4-byte or 2-byte if the C extension is enabled.
